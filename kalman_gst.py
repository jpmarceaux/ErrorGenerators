...
def avg_gs_infidelity(model, noise_model, qubits=1):
    ...
    aei += pygsti.report.entanglement_infidelity(model[('Mdefault')].error_map.to_dense(),
                                                 noise_model[('Mdefault')].error_map.to_dense(), 'pp')
    aei += pygsti.report.entanglement_infidelity(model[('rho0')].error_map.to_dense(),
                                                 noise_model[('rho0')].error_map.to_dense(), 'pp')
...
class ExtendedKalmanFilter():
    ...
    def update_approx(self, circ, count_vec, p0, jac0, hess0, clip_range=[-1, 1], max_itr=1, itr_eps=1e-4, Q=None,
                      R_additional=None):
        """
    ...
    def update_fast(self, circ, count_vec, jac0, hess0, clip_range=[-1, 1], max_itr=1, itr_eps=1e-4, Q=None,
                    R_additional=None):
        """
        Makes an approximate update to the model
        where the jacobian is approximated
...
def make_error_plot(plt_title, model_vectors, true_params, filter_covars, y_range, mle_estimates=None,
                    germ_length_ranges=None, plt_spacing=25):
    """
